...
interface TopRatedMoviesGridProps {
  initialMovies: Movie[];
  initialPage: number;
  totalPages: number;
}
...
async function fetchTopRatedMovies(page: number): Promise<MoviesResponse> {
  const url = `${TMDB_BASE_URL}/movie/top_rated?page=${page}&api_key=${TMDB_API_KEY}`;
...
export function TopRatedMoviesGrid({
  initialMovies,
  initialPage,
  totalPages: initialTotalPages,
}: TopRatedMoviesGridProps) {
  ...
  const loadNextPage = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchTopRatedMovies(nextPage);
