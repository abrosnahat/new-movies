...
async function fetchUpcomingMovies(page: number): Promise<MoviesResponse> {
  const url = `${TMDB_BASE_URL}/movie/upcoming?page=${page}&api_key=${TMDB_API_KEY}`;
...
export function UpcomingMoviesGrid({
  initialMovies,
  initialPage,
  totalPages: initialTotalPages,
}: UpcomingMoviesGridProps) {
  ...
  const loadNextPage = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchUpcomingMovies(nextPage);
